...
- Network target: Cisco DevNet "IOS XR Always-On" sandbox (port 830).
  ...
  Chosen: free, always available, supports NETCONF, no hardware/reservation needed.

- Version control: one shared GitHub repo; each member commits to their own

  folder so participation is visible in history from day one.
...
| 2 Jul | First drafts of Docker env + all playbooks            |
...
(Filled in after running playbooks — see next commit.)
...
- [OPEN] retrieve_info playbook saved as retrieve_info.yaml; all other playbooks
  ...
  use .yml. Owner: M4 (Ain) to confirm/rename for consistency.
